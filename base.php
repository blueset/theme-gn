<?php get_template_part('templates/head'); ?>
<body <?php body_class(); ?>>


  <!--[if lt IE 9]><div class="alert">Your browser is <em>ancient!</em> <a href="http://browsehappy.com/">Upgrade to a different browser</a> or <a href="http://www.google.com/chromeframe/?redirect=true">install Google Chrome Frame</a> to experience this site.</div><![endif]-->

  <?php
    do_action('get_header');
    // Use Bootstrap's navbar if enabled in config.php
    // if (is_home()) {get_template_part('templates/header-home');} else {
    if (current_theme_supports('bootstrap-top-navbar')) {
      get_template_part('templates/header-top-navbar');
    } else {  
      get_template_part('templates/header');
    }
    // }
  ?>

  <div class="wrap container" role="document">
<script src="//1a23.com/cca.js"></script>
    <div class="content row">
      <div class="main <?php echo roots_main_class(); ?>" role="main">
<?php if (is_home()) { ?>

<style>
.widget h3{
display:none;
}

.widget_text{
float:right;
width:38%;
}

.widget_rss > div > ul > li {
list-style: none;
padding-bottom: 1px;
border-bottom: 1px #ccc solid;
}

.widget_rss > div > ul > li:last-child {
border-bottom: 0px;
}

.widget_rss ul{
margin: 0 0 10px 10px;
width:59%;
}

.rss_image{
float: left;
width: 31%;
padding: 0 0 3px 3px;
}


</style>
<article class="post post-header">
  <div class="cont-preview">
    <?php dynamic_sidebar('sidebar-primary'); ?>
  </div>
  <footer>
    <div class="pull-right">
      <a href="https://ilove.1a23.com">1A23 Labs</a><br />
      <a href="http://blueset.deviantart.com/">Eana's DeviantArt</a><br />
	<a href="https://1a23.com/lyricova">Project Lyricova</a><br />
    </div> 
    <h3>Looking for something new?</h3>
    生きているだから、何かが言えます。
  </footer>
</article>

<?php } ?>

        <?php include roots_template_path(); ?>



      </div><!-- /.main -->
      <?php if (roots_display_sidebar()) : ?>
      <aside class="sidebar <?php echo roots_sidebar_class(); ?>" role="complementary">
        <?php include roots_sidebar_path(); ?>
      </aside><!-- /.sidebar -->
      <?php endif; ?>
    </div><!-- /.content -->
  </div><!-- /.wrap -->



  <?php get_template_part('templates/footer'); ?>

</body>
</html>